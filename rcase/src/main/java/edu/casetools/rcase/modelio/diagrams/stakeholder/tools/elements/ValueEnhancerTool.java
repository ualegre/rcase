 ...
 * entity.
 */
public class ValueEnhancerTool extends ElementTool {
	...
	String name = I18nMessageService.getString("Names.ValueEnhancer");
	return ElementUtils.getInstance().createClass(RCaseModule.getInstance(), adaptElement(element), session, name,
		RCaseStereotypes.STEREOTYPE_VALUE_ENHANCER);
    }
	...
	if ((null != graph) && (!graph.isEmpty()) && (graph.get(0) instanceof IDiagramNode)) {
	    ...
	    dnode.setProperty("FILLCOLOR", RCaseColours.BLUE1);
	}
