 ...
 * entity.
 */
public class SoftGoalTool extends ElementTool {
    ...
    public Class createOwnElement(IModelingSession session, MObject element) {
		...
		return (Class) ElementUtils.getInstance().addStereotype(RCasePeerModule.MODULE_NAME, object, RCaseStereotypes.STEREOTYPE_SOFT_GOAL);
	...
	if ((null != graph) && (!graph.isEmpty()) && (graph.get(0) instanceof IDiagramNode)) {
	    ...
	    dnode.setProperty("FILLCOLOR", RCaseColours.PURPLE2);
	}
