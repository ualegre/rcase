 ...
 * 
 */
package edu.casetools.rcase.modelio.diagrams.usecase.tools;
...
/**
 * The Class UseCaseTool is the tool for creating a Use Case.
 */
public class UseCaseTool extends ElementTool {
	...
	return ElementUtils.getInstance().createUseCase(RCaseModule.getInstance(), adaptElement(element), session, name,
	...
	if ((null != graph) && (!graph.isEmpty()) && (graph.get(0) instanceof IDiagramNode)) {
	    ...
	    dnode.setProperty("SHOWSTEREOTYPES", "NONE");
	    dnode.setProperty("INNERSHOWSTEREOTYPES", "NONE");
	}
