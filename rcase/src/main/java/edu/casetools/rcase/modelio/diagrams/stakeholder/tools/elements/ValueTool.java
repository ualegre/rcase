 ...
 * entity.
 */
public class ValueTool extends ElementTool {
	...
	String name = I18nMessageService.getString("Names.Value");
	return DiagramUtils.getInstance().createClass(adaptElement(element), session, name,
		RCaseStereotypes.STEREOTYPE_VALUE);
    }
	...
	if ((null != graph) && (!graph.isEmpty()) && (graph.get(0) instanceof IDiagramNode)) {
	    ...
	    dnode.setProperty("FILLCOLOR", RCaseColours.BLUE2);
	}
