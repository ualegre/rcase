 ...
 * entity.
 */
public class ResourceTool extends ElementTool {
	...
	String name = I18nMessageService.getString("Names.Resource");
	return DiagramUtils.getInstance().createClass(adaptElement(element), session, name,
		RCaseStereotypes.STEREOTYPE_RESOURCE);
    }
	...
	if ((null != graph) && (!graph.isEmpty()) && (graph.get(0) instanceof IDiagramNode)) {
	    ...
	    dnode.setProperty("FILLCOLOR", RCaseColours.PURPLE0);
	}
